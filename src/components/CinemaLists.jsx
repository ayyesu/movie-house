...
const CinemaLists = ({
    ...
    return (
        <>
            <div className='mx-4 flex h-fit flex-col gap-4 rounded-md bg-slate-800 text-white drop-shadow-xl sm:mx-8 sm:p-6'>
                <form
                    className='flex flex-wrap items-center justify-between gap-x-4 gap-y-2'
                    onSubmit={handleSubmit(onAddCinema)}
                >
                    <h2 className='text-2xl font-bold'>Cinema Lists</h2>
                    {auth.role === 'admin' && (
                        <div className='flex w-fit grow sm:justify-end'>
                            ...
                            <button
                                disabled={isAdding}
                                className='flex items-center whitespace-nowrap rounded-r-md bg-slate-500 from-indigo-600 to-blue-500 px-3 py-1 font-medium text-white hover:from-indigo-500 hover:to-blue-400 disabled:from-slate-500 disabled:to-slate-400'
                            >
                                {isAdding ? 'Processing...' : '+'}
                            </button>
                        </div>
