...
const Search = () => {
    ...
    const [unreleasedCheckedShowtimes, setUnreleasedCheckedShowtimes] =
        useState(0);
    const [isFetchingShowtimesDone, setIsFetchingShowtimesDone] =
        useState(false);
    ...
    const onDeleteCheckedShowtimes = async () => {
        ...
        toast.success(
            `Delete ${successCounter} checked showtimes successful!`,
            {
                position: 'top-center',
                autoClose: 2000,
                pauseOnHover: false,
    ...
    const onReleaseCheckedShowtimes = async () => {
        ...
        let successCounter = 0;
        let errorCounter = 0;
        const releasePromises = checkedShowtimes.map(
            async (checkedShowtime) => {
                try {
        ...
        toast.success(
            `Release ${successCounter} checked showtimes successful!`,
            {
                position: 'top-center',
                autoClose: 2000,
                pauseOnHover: false,
            ...
            <div className='mx-4 flex h-fit flex-col gap-2 rounded-lg bg-gradient-to-br from-indigo-200 to-blue-100 p-4 drop-shadow-xl sm:mx-8 sm:p-6'>
                ...
                <div className='flex items-end'>
                    ...
                    {isFetchingShowtimesDone && (
                        <div className='ml-auto flex items-center gap-1 px-1 text-sm font-medium'>
                            ...
                            Showing {filteredShowtimes.length} filtered
                            showtimes
                        </div>
                    )}
                </div>
                ...
                >
                    ...
                    >
                        ...
                        {sortCinema === 1 && (
                            <ChevronUpIcon className='ml-auto w-6 h-6' />
                        )}
                        {sortCinema === -1 && (
                    ...
                    {isFetchingShowtimesDone &&
                        filteredShowtimes.map((showtime, index) => {
                            const showtimeDate = new Date(showtime.showtime);
                            const year = showtimeDate.getFullYear();
                            const month = showtimeDate.toLocaleString(
                                'default',
                                {month: 'short'},
                            );
                            const day = showtimeDate
                            ...
                            const minutes = showtimeDate
                                ...
                                .toString()
                                .padStart(2, '0');
                            const isCheckedRow = checkedShowtimes.includes(
                                showtime._id,
                            );
                            return (
                                <Fragment key={index}>
                                    <div
                                        className={`flex items-center justify-center border-t-2 border-indigo-200 ${
                                            isCheckedRow &&
                                            'border-white bg-blue-200 text-blue-800'
                                        }`}
                                    >
                                        <input
                                            id={showtime._id}
                                            type='checkbox'
                                            className='h-6 w-6'
                                            checked={checkedShowtimes.includes(
                                                showtime._id,
                                            )}
                                            onChange={(e) => {
