...
const Showtime = () => {
    ...
    useEffect(() => {
        // Your existing code for sorting selected seats

        // Calculate the total price based on the number of selected seats
        const newTotalPrice = sortedSelectedSeat.length * showtime.price;
        setTotalPrice(newTotalPrice);
    }, [selectedSeats, showtime.price]);
    ...
    if (row) {
        for (
            let k = 64;
            k <= (row.length === 2 ? row.charCodeAt(0) : 64);
            k++
        ) {
            for (
                let i = 65;
    ...
    const filteredSeats = showtime?.seats?.filter((seat) => {
        return (
            (!filterRow ||
                filterRow.map((row) => row.value).includes(seat.row)) &&
            (!filterColumn ||
                filterColumn
    ...
    return (
        <div className='flex bg-cover min-h-screen flex-col gap-4 bg-gradient-to-br from-indigo-900 to-blue-500 pb-8 sm:gap-8'>
            <Navbar />
            <div className='mx-4 h-fit text-black rounded-lg bg-slate-800 from-indigo-200 to-blue-100 p-4 drop-shadow-xl sm:mx-8 sm:p-6'>
                {showtime.showtime ? (
                    <>
                        <ShowtimeDetails
                            showtime={showtime}
                        ...
                        <div className='mx-auto mt-4 flex flex-col items-center rounded-lg bg-slate-600 from-indigo-100 to-white p-4 text-center drop-shadow-lg'>
                                        ...
                                        <div className='flex items-center'>
                                            ...
                                            {colNumber.map((col, index) => {
                                                return (
                                                    <div
                                                        key={index}
                                                        className='flex text-black h-8 w-8 items-center'
                                                    >
                                                        <p className='w-8 font-semibold'>
                                                            {col}
                                                        </p>
                                                    </div>
                                                );
                        ...
                        {auth.role === 'admin' && (
                            <>
                                <h2 className='mt-4 text-2xl p-4 rounded-md bg-gray-600 font-bold'>
                                    Booked Seats
                                </h2>
